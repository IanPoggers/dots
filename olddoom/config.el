...
(setq
 ...
 ;;my/variable "Iosevka Nerd Font Mono"
 ;;my/bullet "Iosevka Nerd Font Mono"
 ;;my/pretty "Iosevka Nerd Font Mono"
 my/variable "Inter"
 my/bullet "Inter"
 my/pretty "Inter"
 )
...
(setq
 ...
 initial-buffer-choice (f-join org-directory "notes.org"))
...
(use-package! pdf-tools
  :config
  ;; NOTE If this isn't run i have to re-do it every time i syn doom :(((
  (pdf-tools-install))
...
(use-package! org
  ...
  (require 'cl-lib)
  (setq org-custom-properties '("AUTHOR" "EXPORT_LATEX_CLASS" "EXPORT_LATEX_CLASS_OPTIONS" ))
  (cl-pushnew org-default-properties org-custom-properties)
...
(after! org
  (setq org-priority-faces
        '((65 . error) (66 . warning) (67 . warning) (68 . shadow))))
...
;;;;; org-clock-goto auto narrows to the subtree
(after! org
...
(map! :map 'org-agenda-mode-map
      ;; I find the `H` for holidays thing annoying.
      :e "H" nil
      :e "n" #'org-agenda-next-item
      :e "p" #'org-agenda-previous-item
...
(after! org
  (setq
   ...
   '((:name "" :time-grid t)
     ...
     (:name "\nClass (unscheduled)" :and (:tag "class" :not (:habit t)))
     (:name "\nMisc"
...
(use-package! typst-ts-mode
  :mode "\\.typ\\'"
  :config
  (add-hook 'typst-ts-mode (λ! (hl-line-mode -1)))
...
(use-package! ultra-scroll
  ...
  (setq
   scroll-conservatively 80 ; or whatever value you prefer, since v0.4
   ultra-scroll-hide-cursor 0.5
   ;; NOTE important: scroll-margin>0 not yet supported
   scroll-margin 0))

(after! evil
  (setq evil-scroll-count 18)
  (setq-default evil-scroll-count 18))

(add-hook 'doom-first-buffer-hook
          (lambda () (setq-default evil-scroll-count 18)))

(add-hook 'after-change-major-mode-hook
          (lambda () (setq evil-scroll-count 18)))
;;; custom bindings
;;;
...
(after! corfu (map! :map corfu-map  "M-<TAB>" #'+corfu/move-to-minibuffer))

(after! cdlatex (setq cdlatex-simplify-sub-super-scripts nil))

(after! org
  (setq org-clock-goto-may-find-recent-task nil))

;; NOTE some convenient embark actions
